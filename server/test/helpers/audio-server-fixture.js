...
module.exports = function createAudioServerFixture({ lineCount = 1000, texts, allowCueChanges = false } = {}) {
  ...
  loaded._compile(source.replace(/startServer\(\);\s*$/, `
    module.exports = function (lineCount, texts, allowCueChanges) {
      ...
      session.lines = Array.from({ length: texts?.length ?? lineCount }, (_, i) => {
        const text = texts?.[i] ?? '這是第' + i + '句舞台演出的台詞，需要持續跟隨演員說話';
        return { id: 'line-' + i, type: LINE_TYPES.DIALOGUE, text, translations: { primary: text } };
      });
      ...
      persistSessionCurrentIndexSoon = (s) => {
        if (!allowCueChanges) throw new Error('Unexpected cue change in audio load test');
        cueChanges.push(s.currentIndex);
      };
      ...
      return {
        server, session, stream, user, sent, connect, send: connect(), cueChanges,
  ...
  `), filename);
  return loaded.exports(lineCount, texts, allowCueChanges);
};
