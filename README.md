...
- 快取字幕的正規化文字、開頭、句尾、下一格位置、完整句重複位置及對齊索引。字幕內容或音樂等標記有變才重建分析；音訊封包不讀取或重算劇本。
- 以保留字序的連續編輯距離對齊辨識文字與劇本，追蹤格內進度，支援繁簡中文、單字台詞、少量漏字、跨格連續說話，以及一句被拆成多個辨識片段。
- 格內進度達 70% 且剩餘不超過 3 字時，若比對分數足夠，即提前準備下一格開頭。已確認句尾，或接近句尾後收到新的起音時，下一格有區別的 2–12 字開頭可直接確認換格，不必等整句定稿。相似開頭須等待更多內容；音樂與舞台指示不使用此短開頭捷徑。
- 起音只作為輔助證據，音量變大不再直接換格。起音偵測維持至少 120 ms 的低音量及至少 40 ms 的持續高音量。連續說話也可從同一辨識片段新增的開頭文字確認跨格，無須等待停頓。
...
- 以辨識片段 ID、連線 ID 與音訊時間處理過期結果。同一片段的 final 不會把已前進的位置拉回；手動定位、切換場次及重連會清除舊的跟戲依據。
- 辨識服務的 final 只表示該片段定稿，不表示字幕格說完。舞台指示不因 final 自動推進；無聲舞台提示仍需人工 cue 或後續串接獨立舞台訊號。

控制端會顯示比對分數、目前格內進度、準備的下一格開頭、重新定位原因與候選位置。進度是已辨識文字在字幕內的位置估計，不是倒數計時；分數是文字對齊分數，不是正確機率。

目前沒有加入本機人聲分類或音素辨識模型，開頭確認仍依賴雲端的暫定辨識文字。相較於舊版只靠起音預推，現在可能多等一小段辨識時間，以避免掌聲／音效單獨觸發換格；不保證首字零延遲。測試可使用模擬事件，不需要事先有錄音；真實演出延遲、誤跳與跳詞恢復時間仍需現場驗證。若未來有錄音，可再作為驗收素材。
...
測試涵蓋劇本分析快取與編輯失效、句尾準備、短開頭確認、相似開頭、起始重複台詞、連續跨格、句中停頓、單字、跳段、回跳、延遲 final、片段修訂、音樂／舞台指示、手動定位、顯示來源及音訊打包；包含實際伺服器的收音到換格與跳段校正流程，不會呼叫付費辨識服務。實際聲學準確度與演出延遲需另以現場素材驗收。
