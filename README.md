...
在控制端先載入劇本、選好起始位置，再按「自動」並允許麥克風。觀眾、提詞與投影端在自動模式都顯示編輯好的劇本字幕，收音只用於定位。若收音已停止，可再次按「自動」重新啟動。手動點選字幕或按上一句／下一句會重新定位並繼續跟戲；需要停止跟戲時請切換「手動」。

自動跟戲由 `server/src/auto-follow.js` 負責：

- 以保留字序的連續編輯距離對齊辨識文字與劇本，追蹤格內進度，支援繁簡中文、單字台詞、少量漏字、跨格連續說話，以及一句被拆成多個辨識片段。
- 只有核對到目前格句尾才預備下一格。起音需要先有至少 120 ms 的低音量，再有至少 40 ms 的持續高音量；尚未核對句尾的停頓不會推格。音樂區段與舞台指示不使用起音預推。
- AudioWorklet 每 20 ms 傳送一批音訊；啟動時積存超過 250 ms 的封包仍供辨識使用，但不觸發起音預推。音量診斷更新有節流，不隨每個音訊封包重繪。
- 平時搜尋附近台詞；無法符合時，較長的語音可擴大至整場劇本重新定位。跳段／回跳需要較強證據；暫定結果需隨後續內容再次支持，明確且唯一的 final 可直接校正。相似候選無法區分時顯示「重新定位」，維持目前字幕。
...
控制端會顯示比對分數、重新定位原因與「準備第 N 格」。分數是文字對齊分數，不是正確機率。

目前的快速預推仍使用音量起伏，並非本機人聲分類或音素辨識模型。確認句尾後出現未標註的掌聲／音效，仍可能誤觸一次預推；後續語音會繼續核對。沒有停頓的跨格，以及沒有足夠前文的意外跳詞，切換時間仍受雲端 interim 延遲影響，不保證首字零延遲。建議使用調音台人聲輸入，並以完整排練錄音量測每格首字與實際顯示時間、誤跳、跳詞恢復時間及人工介入次數。
