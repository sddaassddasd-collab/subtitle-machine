...
3. 直接貼上劇本文字並點選「使用 OpenAI 拆解字幕」，系統會呼叫 `gpt-4o-mini` 將台詞拆成適合字幕的句子，舞台指示與角色頭銜會被濾除。
...
13. 控制端可輸入「辨識主題 / 術語提示」；後端會把這份提示同時套用到 Realtime 快通道、雙通道精修與後修正，讓專有名詞與領域詞彙更穩定。
14. 若「辨識主題 / 術語提示」留白，系統不會送出額外主題 prompt，行為等同目前預設辨識流程。
15. 控制端收音會保留雙聲道輸入並混成 mono，避免 `BlackHole 2ch` 或其他立體聲來源只吃到單邊聲道。
16. 當語言是 `zh`（或 `zh-*`）時，後端會做 OpenCC（簡轉繁，台灣用字）正規化，預設不送額外轉錄提示詞，避免提示詞內容誤出現在字幕。
17. 劇本字幕清單每行可勾選「此處有音樂」；先勾起點再勾終點，控制端會自動把中間整段標成音樂區段，檢視端在該範圍內會固定顯示「此處有音樂」提示。
...
- 後端以記憶體儲存字幕狀態，若伺服器重啟需重新貼上劇本文字並解析。
