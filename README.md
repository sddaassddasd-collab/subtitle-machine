...
5. 控制端可分別指定「檢視端預設語言」與「投影端播放語言」；檢視端初次進入會先套用預設語言，但觀眾之後仍可自行切換，且不會再被控制端後續改動覆蓋。投影端則會持續跟著控制端指定的播放語言。
6. 角色資訊會保留在字幕資料裡，是否以顏色區分角色改由控制端切換，檢視端與投影端會同步套用。
7. 需要暫時關閉觀眾字幕時，點「遮蔽檢視端字幕」即可讓檢視端畫面變成空白，但控制端仍可瀏覽全文、以 `Command + F` 搜尋並點擊跳轉。
8. 若要手動備份場次，可在控制端使用「匯出場次備份 JSON」；備份會保留原本 `sessionId`、viewer/projector token、預設播放語言、語言、所有儲存格與字幕內容。
9. 若要還原備份，可在首頁或控制端使用「匯入場次備份 JSON」；若備份中的 `sessionId` 已存在，系統會拒絕匯入，避免悄悄改號。
