...
13. 控制端可額外勾選「辨認講者」；Deepgram 模式會在 Nova-3 串流直接啟用原生 diarization，依 final word 的 speaker 標籤切分字幕；OpenAI 備援模式則會把最近幾行音訊視窗送到 `gpt-4o-transcribe-diarize`。兩種模式都會在檢視端以不同顏色標示不同講者。
