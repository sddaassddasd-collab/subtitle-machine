...
- 起始位置未確認、附近候選模糊、比對不符或出現遠處完整句時，較長的語音可擴大至整場劇本重新定位。跳段／回跳需要較強證據；暫定結果需隨後續內容再次支持，明確且唯一的 final 可直接校正。保留至多三個候選位置；無法區分時維持目前字幕，矛盾的語音會取消原本準備。
...
控制端會顯示比對分數、目前格內進度、準備的下一格開頭、預推待核對狀態、重新定位原因與候選位置。進度是已辨識文字在字幕內的位置估計，不是倒數計時；分數是文字對齊分數，不是正確機率。
