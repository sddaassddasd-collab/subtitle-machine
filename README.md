...
即時語音模式下，檢視端會改用獨立的即時翻譯語言選單（原文、English、日本語、한국어、Español、Français、Deutsch）。後端只會將已有觀眾需求的語言送交 OpenAI 翻譯，且每個 final 段落、每種語言只產生一份共用結果。新加入的觀眾可補齊最近 10 個段落；控制端會顯示各翻譯語言的即時需求人數。翻譯尚未產生時，檢視端會繼續顯示原文，不插入「翻譯中」提示；翻譯完成後直接換成譯文。準備中、失敗與原文回退提示會使用觀眾選擇的目標語言。離開即時模式後，檢視端會恢復原本的劇本語言選擇。
