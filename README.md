...
即時語音模式下，檢視端會改用獨立的即時翻譯語言選單（原文、English、日本語、한국어、Español、Français）。後端只會將已有觀眾需求的語言送交 OpenAI 翻譯，且每個 final 段落、每種語言只產生一份共用結果。新加入的觀眾可補齊最近 10 個段落；控制端會顯示各翻譯語言的即時需求人數。離開即時模式後，檢視端會恢復原本的劇本語言選擇。
